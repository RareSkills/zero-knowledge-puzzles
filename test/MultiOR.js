...
describe("MultiOR -- At least one element is non-zero", function() {
    this.timeout(100000);

    let circuit;

    before(async () => {
        circuit = await wasm_tester(path.join(__dirname, "../MultiOR/", "MultiOR.circom"));
        await circuit.loadConstraints();
    });

    it("Should return true if at least one element is 1", async () => {
    ...
    it("Should return false if all elements are 0", async () => {
        witness = await circuit.calculateWitness({
            "in": [0, 0, 0, 0]
        }, true);
        out = witness[1];
